...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for vali in qi.curDomain():
            for valj in qj.curDomain():
                diag = abs(vali - valj) == abs(i - j)
                if not diag and vali != valj:
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        # varsToAssign = self.scope()
        # varsToAssign.remove(var)
        # return findvals(varsToAssign, [(var, val)], finalTest, partialTest)

        assigned_count = 1 if val in self._required else 0
        ...
        for v in self.scope():
            ...
            if v.isAssigned():
                ...
                found = False
                for candidiate in v.curDomain():
                    if candidiate in self._required:
                        found = True
                        break
                if found:
                    unassigned_candidate += 1

        if assigned_count > self._ub or \
           unassigned_candidate < max(0, self._lb-assigned_count):
            return False
        return True
